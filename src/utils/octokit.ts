...
export function setupOctokit(githubToken = "", personalAccessToken = "") {
  ...
  if (personalAccessToken === "") {
    action.fail(
      `Missing personal access token (https://github.com/settings/tokens/new) with "repo" scope. Add it as a secret named "PERSONAL_ACCESS_TOKEN" (https://github.com/${context.repo.owner}/${context.repo.repo}/settings/secrets/actions/new).`,
    );
  }
