...
export async function reRunLastWorkFlowIfRequired() {
  if (context.eventName === "pull_request") {
    action.debug("rerun not required for event - pull_request");
    return;
  }
  ...
  if (runs.data.total_count > 0) {
    ...
    if (isLastWorkFlowFailed) {
      action.debug(`Rerunning build run ${run}`);
...
async function getSelfWorkflowId(): Promise<number> {
  ...
  if (!workflow) {
    throw new Error(
      `Unable to locate this workflow's ID in this repository, can't retrigger job..`,
    );
  }
  return workflow.id;
}
